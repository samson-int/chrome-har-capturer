...
function fromPage(page) {
    ...
    for (var requestId in page.objects) {
        ...
        var duration = toMilliseconds(object.responseFinished - timing.requestTime);
        ...
        var receiveTime = duration - timing.receiveHeadersEnd;
        ...
        var performanceEntry = undefined;
        var encodedBodySize = undefined;
        var decodedBodySize = undefined;

        for (var i = 0; i < page.performance.entries.length; i++) {
            if (page.performance.entries[i].name === object.requestMessage.request.url) {
                performanceEntry = page.performance.entries[i];

                if (performanceEntry.type === 'navigate') {
                    duration = performanceEntry.responseEnd;
                }
                else {
                ...
                decodedBodySize = performanceEntry.decodedBodySize;
            };
        };
        ...
        entries.push({
            ...
            'response': {
                ...
                'bodySize': encodedBodySize || bodySize,
                '_transferSize': transferSize,
                'content': {
                    'size': decodedBodySize || object.responseLength,
            ...
            '_performanceEntry': performanceEntry
        });
    }
    // outcome
    return {
        'info': {
            ...
            'pageTimings': {
                ...
                'onLoad': onLoad,
                'timing': page.performance.timing
            }
        },
        'entries': entries
    };
